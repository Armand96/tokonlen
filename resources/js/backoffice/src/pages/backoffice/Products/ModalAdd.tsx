...
export const ModalAdd = ({
    ...
    const [selectedCategories, setSelectedCategories] = useState<Dropdown>({
        value: "",
        label: "",
    });
    const [subCategoriesOptions, setSubCategoriesOptions] =
        useState<Dropdown[]>();
    const [selectedSubCategories, setSelectedSubCategories] =
        useState<Dropdown>({ value: "", label: "" });
    ...
    useEffect(() => {
        setLoading(true);
        Promise.all([
            GetCategories(`?data_per_page=100000&is_active=1`),
            getLinkType(`?data_per_page=10000&is_active=1`),
            GetBrands(),
        ])
            .then((res) => {
                setCategoriesOptions(
                    HelperFunction.FormatOptions(res[0].data, "name", "id"),
                );
                setLinkOptions(
                    HelperFunction.FormatOptions(res[1].data, "name", "id"),
                );
    ...
    const postData = () => {
        ...
        PostProducts(postData).then(async (res) => {
            try {
                ...
                formData.image_files.map(async (file: any) => {
        ...
        >
            <div className="m-3 sm:mx-auto flex flex-col bg-white shadow-sm rounded">
                ...
                <div className="p-4 overflow-y-auto w-[70vw]">
                    ...
                    <FormInput
                        ...
                        onChange={(e) =>
                            setFormData({
                                ...formData,
                                price: parseInt(
                                    HelperFunction.onlyNumber(e.target.value),
                                ),
                            })
                        }
                        className="form-input mb-3"
                    ...
                    <div className="mb-4">
                        ...
                        <Select
                            ...
                            onChange={(v) => handleOnSelect(v, "categories")}
                        />
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <Select
                            isDisabled={subCategoriesOptions?.length === 0}
                            className="select2 z-5"
                            options={subCategoriesOptions}
                            value={selectedSubCategories}
                            onChange={(v) =>
                                handleOnSelect(v, "sub_categories")
                            }
                        />
                    </div>
                    ...
                    <div className="mb-2 border px-3 py-4">
                        ...
                        <div className="flex gap-x-4 items-center ">
                            ...
                            <div className="w-3/6 mt-6">
                                <Select
                                    className="select2 h-full"
                                    options={linkOptions}
                                    value={selectedLink}
                                    onChange={(v) => setSelectedLink(v)}
                                />
                            </div>
                        </div>
                        <button
                            ...
                            disabled={!selectedLink || !tempLink ? true : false}
                        >
                            Add Link
                        </button>
                ...
                <div className="flex justify-end p-4 border-t gap-x-4">
                    <button
                        className="btn bg-light text-gray-800"
                        onClick={() => toggleModal()}
                    >
                        Close
                    </button>
