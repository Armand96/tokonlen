...
class ProductCController extends Controller
{
    public function getListActiveProduct(Request $req)
    {
        ...
        $query->where('is_active', true)->with(['image', 'discount', 'variant.images', 'variant.links.linkType', 'variant.discount', 'category.parentCat', 'category.subCat', 'links.linkType']);
    ...
    public function getOneActiveProductWithProducts(Product $product)
    {
        // DB::enableQueryLog();
        if ($product) {
            $product = Product::with(['images', 'variant.images', 'variant.links.linkType','links.linkType.', 'discount', 'category.parentCat', 'category.subCat', 'variant' => function($q) {
